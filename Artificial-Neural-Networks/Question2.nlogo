breed [ lattice-nodes lattice-node ]

patches-own [ node ]
lattice-nodes-own [ weights ]
...
to setup-lattice
  ...
  while [ counter < (lattice-size ^ 2) ] [
    create-lattice-nodes 1 [
    ...
    ifelse patch-x = lattice-size [
      set patch-x 0
      set patch-y patch-y + 1
    ]
    [
      set patch-x patch-x + 1
    ]
...
to-report calculate-distance [ current-input-vector current-lattice-node ]
  let counter 0
  let dist 0
  
  while [ counter < length current-input-vector ] [
    set dist dist + ( (first current-input-vector - first current-lattice-node) ^ 2 )
    set current-input-vector but-first current-input-vector
    set current-lattice-node but-first current-lattice-node
  ]
...
to find-bmu
  ...
  set bmu best-fit
  ask bmu [ adjust-weight-inside-bmu-radius current-vector ]
end

to-report neighbourhood-radius
